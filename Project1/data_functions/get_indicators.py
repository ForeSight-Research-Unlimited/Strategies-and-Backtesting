...
def getRSI(data1):
    ...
    try:
        ...
        data1["Change"] = data1["Close"].copy().diff()
...
def getBB(data1, interval = 10):
    ...
    try:
        ...
        data1BB["1STDUP"] = data1["Typical"].rolling(window = interval).mean() + data1["Typical"].rolling(window = interval).std()
        data1BB["2STDUP"] = data1["Typical"].rolling(window = interval).mean() + 2*(data1["Typical"].rolling(window = interval).std())
        data1BB["1STDDN"] = data1["Typical"].rolling(window = interval).mean() - data1["Typical"].rolling(window = interval).std()
        data1BB["2STDDN"] = data1["Typical"].rolling(window = interval).mean() - 2*(data1["Typical"].rolling(window = interval).std())
